...
/// And unlike that package, it works! And it does not use the code of Dart team, which changes over time.
///
/// {@template fs.support}
/// # Supported classes
/// * [File]
...
abstract class FileSystemEntity implements uber.FileSystemEntity {
  ...
  FileStat statSync() {
    final Map<String, dynamic> map =
        json.decode(window.localStorage[_statKey]!);
    return FileStat._(
  ...
  void _stat(String type, [DateTime? date]) {
    ...
    window.localStorage[_statKey] = json.encode({...stat, type: '$dateOrNow'});
    try {
...
class FileStat implements uber.FileStat {
  ...
  FileStat._(this.accessed, this.changed, this.mode, this.modified, this.size,
      this.type);
  ...
  void createSync({bool recursive = false}) {
    ...
    if (!existsSync()) {
      if (!recursive) {
        ...
        for (Directory dir = this;
            !dir.existsSync() && dir.uri.pathSegments.isNotEmpty;
            dir = dir.parent) {
          dir.createSync();
        }
      }
    ...
    return [
      for (final key in window.localStorage.keys)
        if (key != storageKey &&
            ...
            Uri.tryParse(key.replaceFirst(RegExp('/\$'), ''))
                    ?.pathSegments
                    .length ==
                ('${absolute.uri}' == '/'
                    ? 1
                    : absolute.uri.pathSegments.length))
...
class File extends FileSystemEntity implements uber.File {
  ...
  Future<File> create({bool recursive = false, bool exclusive = false}) {
    if (existsSync()) {
      return Future.error(
          FileSystemException('File already exists', uri.toString()));
    }
  ...
  Uint8List readAsBytesSync() {
    return Uint8List.fromList(window.localStorage[uri.toString()]
            ?.split('-')
            .map(int.parse)
            .toList() ??
        []);
  ...
  String readAsStringSync({Encoding encoding = utf8}) {
    return encoding.decode(window.localStorage['${absolute.uri}']!
        .split('-')
        .map(int.parse)
        .toList());
  }
  ...
  Future<File> writeAsBytes(List<int> bytes,
      {uber.FileMode mode = FileMode.write, bool flush = false}) {
  ...
  Future<File> writeAsString(String contents,
      {uber.FileMode mode = FileMode.write,
